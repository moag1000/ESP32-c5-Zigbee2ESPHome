...
static void switch_timer_noop(TimerHandle_t timer);
...
esp_err_t zb_multi_pan_init(void)
{
    ...
    /* Create switch timer (auto-switch callback placeholder) */
...
/**
 * @brief No-op timer callback placeholder for auto-switch (not yet implemented)
 */
static void switch_timer_noop(TimerHandle_t timer)
{
    (void)timer;
...
static void publish_status_mqtt(void)
{
    ...
    if (json_str != NULL) {
         ...
         * MQTT_TOPIC_STATUS is a constant -> strdup.
         * json_str from cJSON_PrintUnformatted -> already heap, transfer directly. */
        char *topic_heap = strdup(MQTT_TOPIC_STATUS);
        if (topic_heap == NULL) {
            ESP_LOGE(TAG, "Failed to strdup multi-pan status topic");
            free(json_str);
            return;
        }
