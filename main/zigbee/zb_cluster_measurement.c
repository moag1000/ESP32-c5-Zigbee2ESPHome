...
esp_err_t zb_cluster_measurement_init(void)
{
    ...
    if (s_illuminance_states == NULL) {
        s_illuminance_states = mem_ng_calloc(ZB_STATE_MAX_ILLUMINANCE,
    ...
    if (s_illuminance_addrs == NULL) {
        s_illuminance_addrs = mem_ng_calloc(ZB_STATE_MAX_ILLUMINANCE,
                                            sizeof(uint16_t),
                                            MEM_CAP_PSRAM);
    ...
    if (!s_illuminance_states || !s_illuminance_addrs) {
        ...
        mem_ng_free(s_illuminance_states);
        s_illuminance_states = NULL;
        mem_ng_free(s_illuminance_addrs);
    ...
    if (s_pressure_states == NULL) {
        s_pressure_states = mem_ng_calloc(ZB_STATE_MAX_PRESSURE,
    ...
    if (s_pressure_addrs == NULL) {
        s_pressure_addrs = mem_ng_calloc(ZB_STATE_MAX_PRESSURE,
                                         sizeof(uint16_t),
                                         MEM_CAP_PSRAM);
        if (!s_pressure_addrs) {
            s_pressure_addrs = mem_ng_calloc(ZB_STATE_MAX_PRESSURE,
    ...
    if (!s_pressure_states || !s_pressure_addrs) {
        ...
        mem_ng_free(s_illuminance_addrs);
        s_illuminance_addrs = NULL;
        mem_ng_free(s_pressure_states);
        s_pressure_states = NULL;
        mem_ng_free(s_pressure_addrs);
    ...
    if (s_pm25_states == NULL) {
        s_pm25_states = mem_ng_calloc(ZB_STATE_MAX_PM25,
    ...
    if (s_pm25_addrs == NULL) {
        s_pm25_addrs = mem_ng_calloc(ZB_STATE_MAX_PM25,
                                     sizeof(uint16_t),
                                     MEM_CAP_PSRAM);
        if (!s_pm25_addrs) {
            s_pm25_addrs = mem_ng_calloc(ZB_STATE_MAX_PM25,
    ...
    if (!s_pm25_states || !s_pm25_addrs) {
        ...
        mem_ng_free(s_pressure_addrs);
        s_pressure_addrs = NULL;
        mem_ng_free(s_pm25_states);
        s_pm25_states = NULL;
        mem_ng_free(s_pm25_addrs);
        s_pm25_addrs = NULL;
        return ESP_ERR_NO_MEM;
    }
...
esp_err_t zb_illuminance_read_value(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Illuminance read request queued, TSN=0x%02X", tsn);
...
esp_err_t zb_pressure_read_value(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Pressure read request queued, TSN=0x%02X", tsn);
...
esp_err_t zb_pm25_read_value(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "PM2.5 read request queued, TSN=0x%02X", tsn);
