...
/**
 * @brief Publish event via event bus
 *
...
static void publish_event_mqtt(const zb_drlc_active_event_t *event, const char *action)
{
    ...
    evt_drlc_load_control_t evt = {
        .ieee_addr = 0,  /* Coordinator/bridge event */
        .short_addr = 0,
        .event_id = event->event.issuer_event_id,
        .criticality_level = event->event.criticality_level,
        .duration_min = event->event.duration_minutes,
        .opt_out_available = s_config.allow_user_opt_out && !event->user_opted_out,
        .json_payload = json_payload,
    };

    esp_err_t ret = event_publish(EVT_DRLC_LOAD_CONTROL, &evt, sizeof(evt));
    if (ret != ESP_OK) {
...
esp_err_t zb_demand_response_deinit(void)
{
    ...
    if (s_event_timer != NULL) {
        xTimerStop(s_event_timer, GW_TIMEOUT_MEDIUM_TICKS);
        xTimerDelete(s_event_timer, GW_TIMEOUT_MEDIUM_TICKS);
        s_event_timer = NULL;
    }
                                            ...
                                            uint8_t signature_type)
{
    ...
    ESP_LOGD(TAG, "ReportEventStatus queued, TSN=0x%02X", tsn);
    return ESP_OK;
}
                                                    ...
                                                    uint8_t num_events)
{
    ...
    ESP_LOGD(TAG, "GetScheduledEvents queued, TSN=0x%02X", tsn);
    return ESP_OK;
}
...
esp_err_t zb_demand_response_publish_events(void)
{
    ...
    offset += snprintf(json_payload + offset, sizeof(json_payload) - offset, "{\"events\":[");

    xSemaphoreTake(s_mutex, GW_TIMEOUT_LONG_TICKS);

    for (size_t i = 0; i < s_active_event_count && (offset < sizeof(json_payload) - 200); i++) {
        zb_drlc_active_event_t *event = &s_active_events[i];
    ...
    /* Publish summary event with event_id=0 to indicate it's a summary */
    evt_drlc_load_control_t evt = {
        .ieee_addr = 0,
        .short_addr = 0,
        .event_id = 0,  /* Summary event */
        .criticality_level = 0,
        .duration_min = 0,
    ...
    return event_publish(EVT_DRLC_LOAD_CONTROL, &evt, sizeof(evt));
}
...
esp_err_t zb_demand_response_publish_status(void)
{
    ...
    /* Publish status event with event_id=0xFFFFFFFF to indicate it's a status message */
    evt_drlc_load_control_t evt = {
        .ieee_addr = 0,
        .short_addr = 0,
        .event_id = 0xFFFFFFFF,  /* Status message indicator */
        .criticality_level = 0,
        .duration_min = 0,
        .opt_out_available = s_config.allow_user_opt_out,
        .json_payload = json_payload,
    };
