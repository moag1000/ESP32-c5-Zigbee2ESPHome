...
esp_err_t zb_groups_init(void)
{
    ...
    /* Allocate group storage using memory manager (auto-selects PSRAM for large allocs) */
    s_groups = mem_ng_calloc(ZB_GROUPS_MAX_COUNT, sizeof(zb_group_t), MEM_CAP_PSRAM);
    if (s_groups == NULL) {
...
esp_err_t zb_groups_deinit(void)
{
    ...
    xSemaphoreTake(s_groups_mutex, GW_TIMEOUT_LONG_TICKS);

    if (s_groups != NULL) {
        mem_ng_free(s_groups);
        s_groups = NULL;
    }
...
esp_err_t zb_groups_remove(uint16_t group_id)
{
    ...
    strncpy(group_name, group->name, ZB_GROUP_NAME_MAX_LEN - 1);
    group_name[ZB_GROUP_NAME_MAX_LEN - 1] = '\0';
...
esp_err_t zb_groups_rename(uint16_t group_id, const char *new_name)
{
    ...
    strncpy(old_name, group->name, ZB_GROUP_NAME_MAX_LEN - 1);
    old_name[ZB_GROUP_NAME_MAX_LEN - 1] = '\0';
...
static esp_err_t send_zcl_add_group(uint16_t group_id, uint64_t ieee_addr)
{
    ...
    esp_zb_zcl_groups_add_group_cmd_t cmd = {
        .zcl_basic_cmd = {
            .dst_addr_u.addr_short = device->proto.zigbee.short_addr,
            .dst_endpoint = device->proto.zigbee.endpoint,
            .src_endpoint = 1,
        },
    ...
    ESP_LOGD(TAG, "Add Group command queued, TSN=0x%02X", tsn);
    return ESP_OK;
}
...
static esp_err_t send_zcl_remove_group(uint16_t group_id, uint64_t ieee_addr)
{
    ...
    /* Thread-safety: Acquire Zigbee lock before API call */
    esp_zb_lock_acquire(GW_TIMEOUT_VERY_LONG_TICKS);
    uint8_t tsn = esp_zb_zcl_groups_remove_group_cmd_req(&cmd);
    esp_zb_lock_release();

    ESP_LOGD(TAG, "Remove Group command queued, TSN=0x%02X", tsn);
    return ESP_OK;
}
