...
esp_err_t zb_scenes_init(void)
{
    ...
    /* Allocate scene storage using memory manager (auto-selects PSRAM for large allocs) */
    s_scenes = mem_ng_calloc(ZB_SCENES_MAX_COUNT, sizeof(zb_scene_t), MEM_CAP_PSRAM);
    if (s_scenes == NULL) {
...
esp_err_t zb_scenes_deinit(void)
{
    ...
    xSemaphoreTake(s_scenes_mutex, GW_TIMEOUT_LONG_TICKS);

    if (s_scenes != NULL) {
        mem_ng_free(s_scenes);
        s_scenes = NULL;
    }
...
esp_err_t zb_scenes_remove(uint8_t scene_id)
{
    ...
    strncpy(scene_name, scene->name, ZB_SCENE_NAME_MAX_LEN - 1);
    scene_name[ZB_SCENE_NAME_MAX_LEN - 1] = '\0';
...
esp_err_t zb_scenes_rename(uint8_t scene_id, const char *new_name)
{
    ...
    strncpy(old_name, scene->name, ZB_SCENE_NAME_MAX_LEN - 1);
    old_name[ZB_SCENE_NAME_MAX_LEN - 1] = '\0';
...
esp_err_t zb_scenes_process_mqtt_recall(const char *topic, const char *payload, size_t len)
{
    ...
    char *scene_name = mem_alloc(len + 1, MEM_CAP_DEFAULT);
    if (scene_name == NULL) {
        return ESP_ERR_NO_MEM;
    }
    ...
    if (json != NULL) {
        ...
        if (name_obj != NULL && cJSON_IsString(name_obj)) {
            mem_ng_free(scene_name);
            scene_name = mem_strdup(name_obj->valuestring, MEM_CAP_DEFAULT);
        }
        cJSON_Delete(json);
    }
...
esp_err_t zb_scenes_publish_state(uint8_t scene_id)
{
    ...
    ESP_LOGI(TAG, "Publishing scene state for '%s'", scene->name);
    ESP_LOGD(TAG, "State: %s", json_str);

    /* Publish via event bus - mqtt_event_handler.c will handle MQTT publishing */
    evt_zb_scene_t evt = {
        .group_id = scene->group_id,
        .scene_id = scene->scene_id,
        .scene_name = scene->name,
    };
    esp_err_t ret = event_publish(EVT_ZB_SCENE_ACTIVATED, &evt, sizeof(evt));
...
esp_err_t zb_scenes_publish_list(void)
{
    ...
    ESP_LOGI(TAG, "Publishing scenes list (%d scenes)", s_scene_count);
    ESP_LOGD(TAG, "Scenes: %s", json_str);

    /* Publish via event bus - mqtt_event_handler.c will handle MQTT publishing
     * Note: For scene list, we publish a scene activated event with scene_id=0
     * to indicate this is a list update, not a specific scene activation
     */
    evt_zb_scene_t evt = {
        .group_id = 0,
        .scene_id = 0,
...
static esp_err_t apply_device_state(const zb_scene_device_state_t *state, uint16_t transition)
{
    ...
    /* Find device by IEEE address using NG device registry */
    device_t *device = device_registry_get(state->ieee_addr);
    if (device == NULL) {
    ...
    esp_zb_zcl_on_off_cmd_t on_off_cmd = {
        .zcl_basic_cmd = {
            .dst_addr_u = {
                .addr_short = device->proto.zigbee.short_addr,
            },
