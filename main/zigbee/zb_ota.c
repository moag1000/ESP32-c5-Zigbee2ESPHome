...
/* Override OTA mount point to use shared LittleFS */
#undef ZB_OTA_LITTLEFS_MOUNT_POINT
#define ZB_OTA_LITTLEFS_MOUNT_POINT "/littlefs/ota_storage"
...
esp_err_t zb_ota_init(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "LittleFS mount failed, OTA storage unavailable: %s",
                 esp_err_to_name(ret));
        /* Continue without filesystem - images can still be added via data */
    }
...
esp_err_t zb_ota_add_image(const char *file_path, zb_ota_image_info_t *image_info)
{
    ...
    xSemaphoreTake(s_ota_mutex, GW_TIMEOUT_LONG_TICKS);
                                 ...
                                 zb_ota_image_info_t *image_info)
{
    ...
    snprintf(file_path, sizeof(file_path), "%s/%04X_%04X_%08lX.ota",
             ZB_OTA_LITTLEFS_MOUNT_POINT,
             header.manufacturer_code,
             header.image_type,
             (unsigned long)header.file_version);
    ...
    if (s_fs_mounted) {
        ...
        ESP_LOGE(TAG, "Filesystem not mounted, cannot store image");
        xSemaphoreGive(s_ota_mutex);
        return ESP_ERR_INVALID_STATE;
    }
...
{
    /* Note: upper bound check removed as uint8_t max == ZB_OTA_MAX_BLOCK_SIZE (255) */
    if (block_size < ZB_OTA_MIN_BLOCK_SIZE) {
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t zb_ota_mqtt_receive_image(const uint8_t *data, size_t len, bool final)
{
    ...
    if (s_upload_buffer == NULL) {
        ...
        /* Use memory manager for large OTA buffer (auto-selects PSRAM) */
        s_upload_buffer = mem_alloc(s_upload_buffer_size, MEM_CAP_PSRAM);
        if (s_upload_buffer != NULL) {
            ESP_LOGI(TAG, "Allocated %u bytes for OTA buffer", s_upload_buffer_size);
        }
        if (s_upload_buffer == NULL) {
...
esp_err_t zb_ota_publish_images(void)
{
    ...
    /* Publish via event bus - mqtt_event_handler will handle MQTT publishing */
    evt_zb_ota_images_t evt = {
        .json_images = json_str,
        .image_count = valid_image_count,
    };
    esp_err_t ret = event_publish(EVT_ZB_OTA_IMAGES_CHANGED, &evt, sizeof(evt));
...
esp_err_t zb_ota_publish_status(const esp_zb_ieee_addr_t ieee_addr)
{
    ...
    /* Prepare event data */
    evt_zb_ota_progress_t evt = {0};
    memcpy(evt.ieee_addr, ieee_addr, sizeof(evt.ieee_addr));
    evt.short_addr = transfer->short_addr;
    evt.state = zb_ota_get_state_string(transfer->state);
    evt.percent = calculate_progress(transfer);
    evt.remaining = transfer->image_size - transfer->file_offset;

    /* Check if transfer is complete (success or failure) */
    bool is_complete = (transfer->state == ZB_OTA_TRANSFER_SUCCESS ||
                        transfer->state == ZB_OTA_TRANSFER_FAILED ||
                        transfer->state == ZB_OTA_TRANSFER_ABORTED);

    uint32_t duration_ms = 0;
    bool success = false;
    if (is_complete) {
        duration_ms = (xTaskGetTickCount() * portTICK_PERIOD_MS) - transfer->start_time;
        success = (transfer->state == ZB_OTA_TRANSFER_SUCCESS);
    }
    ...
    if (is_complete) {
        ...
        evt_zb_ota_complete_t complete_evt = {0};
        memcpy(complete_evt.ieee_addr, ieee_addr, sizeof(complete_evt.ieee_addr));
        complete_evt.short_addr = evt.short_addr;
        complete_evt.success = success;
        complete_evt.duration_ms = duration_ms;
        ret = event_publish(EVT_ZB_OTA_COMPLETE, &complete_evt, sizeof(complete_evt));
    } else {
        /* Publish progress event */
        ret = event_publish(EVT_ZB_OTA_PROGRESS, &evt, sizeof(evt));
    }
...
static esp_err_t mount_littlefs(void)
{
    ESP_LOGI(TAG, "Mounting LittleFS for OTA storage...");
    ...
    /* Create OTA storage directory if it doesn't exist */
    struct stat st;
    if (stat(ZB_OTA_LITTLEFS_MOUNT_POINT, &st) != 0) {
        ESP_LOGI(TAG, "Creating OTA storage directory: %s", ZB_OTA_LITTLEFS_MOUNT_POINT);
        mkdir(ZB_OTA_LITTLEFS_MOUNT_POINT, 0755);
    }
...
static esp_err_t unmount_littlefs(void)
{
    ...
    littlefs_unmount();
    s_fs_mounted = false;
    ESP_LOGI(TAG, "LittleFS unmounted (OTA)");
    return ESP_OK;
}
...
static esp_err_t parse_image_file(const char *file_path, zb_ota_image_info_t *info)
{
    ...
    strncpy(info->header_string, header.header_string, ZB_OTA_HEADER_STRING_MAX_LEN - 1);
    info->header_string[ZB_OTA_HEADER_STRING_MAX_LEN - 1] = '\0';
