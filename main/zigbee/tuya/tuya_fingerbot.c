    ...
    switch (dp->dp_id) {
        ...
        case TUYA_DP_FINGERBOT_BATTERY:
            ...
            if (state->battery > 0) {
                ...
                if (dev != NULL) {
                    dev->proto.zigbee.power_info.current_power_source =
                        ZB_POWER_SOURCE_RECHARGEABLE_BATTERY;
                    if (state->battery <= 5) {
                        dev->proto.zigbee.power_info.current_power_source_level = ZB_POWER_LEVEL_CRITICAL;
                    } else if (state->battery <= 33) {
                        dev->proto.zigbee.power_info.current_power_source_level = ZB_POWER_LEVEL_33_PERCENT;
                    } else if (state->battery <= 66) {
                        dev->proto.zigbee.power_info.current_power_source_level = ZB_POWER_LEVEL_66_PERCENT;
                    } else {
                        dev->proto.zigbee.power_info.current_power_source_level = ZB_POWER_LEVEL_100_PERCENT;
                    }
                    dev->proto.zigbee.power_info.power_info_valid = true;
                }
            }
            break;
                                           ...
                                           const cJSON *json)
{
    ...
    /* Handle click_control: {"click_control": true|false} (DP 108, Fingerbot Plus only) */
    cJSON *click_json = cJSON_GetObjectItem(json, "click_control");
    if (cJSON_IsBool(click_json)) {
        bool enable = cJSON_IsTrue(click_json);
        ret = zb_tuya_fingerbot_set_click_control(short_addr, endpoint, enable);
        if (ret == ESP_OK) {
            ESP_LOGI(TAG, "Set Fingerbot click_control: 0x%04X -> %s",
                     short_addr, enable ? "true" : "false");
            handled = true;
        }
    }
    ...
    if (cJSON_IsString(action_json) && action_json->valuestring != NULL) {
        ...
        if (strcmp(action, "on") == 0) {
            ...
            /* Get IEEE address from NG registry */
            device_t *reset_dev = device_registry_get_by_short_addr(short_addr);
            if (reset_dev != NULL) {
                uint8_t ieee_addr[8];
                device_id_t reset_id = reset_dev->id;
                for (int b = 7; b >= 0; b--) {
                    ieee_addr[b] = (uint8_t)(reset_id & 0xFF);
                    reset_id >>= 8;
                }
...
static cJSON *fingerbot_build_state_json(uint16_t short_addr)
{
    ...
    cJSON_AddNumberToObject(json, "click_count", state->click_count);

    /* Battery: prefer Tuya DP 105, fall back to device_t power info */
    uint8_t battery = state->battery;
    if (battery == 0) {
        device_t *dev = device_registry_get_by_short_addr(short_addr);
        if (dev != NULL) {
            int dev_battery = device_zigbee_get_battery_percent(dev);
            if (dev_battery > 0) {
                battery = (uint8_t)dev_battery;
            }
        }
    }
...
/**
 * @brief Serialize and publish discovery config via event bus
 */
                                           ...
                                           const char *device_name)
{
     ...
     * topic is a stack buffer from caller -> strdup.
     * json_str from json_to_string_and_delete -> already heap, transfer directly. */
    char *topic_heap = strdup(topic);
    if (topic_heap == NULL) {
        ESP_LOGE(TAG, "Failed to strdup discovery topic");
        free(json_str);
        return ESP_ERR_NO_MEM;
    }
...
/**
 * @brief Get user-friendly display name for a device.
 *
 * Returns model name if friendly_name is still the default IEEE hex string
 * (e.g. "0xa4c138cd664a2051"), otherwise returns friendly_name.
 * MQTT topics must still use friendly_name for routing.
 */
static const char *get_display_name(const device_t *device)
{
    const char *name = device->friendly_name;
    if (name[0] == '0' && name[1] == 'x' && device->model[0] != '\0') {
        return device->model;
    }
    return name;
}
 ...
 */
static void add_device_info(cJSON *config, const device_t *device)
{
    char ieee_str[MQTT_IEEE_ADDR_STR_LEN];
    device_id_to_str(device->id, ieee_str, sizeof(ieee_str));
    ...
    /* Device name - use friendly_name, fall back to model or IEEE.
     * Detect default IEEE hex string (e.g. "0xa4c138cd664a2051") as non-friendly. */
    const char *name = device->friendly_name;
    if (name[0] == '\0' ||
        (name[0] == '0' && name[1] == 'x' && device->model[0] != '\0')) {
        name = (device->model[0] != '\0') ? device->model : ieee_str;
    }
    cJSON_AddStringToObject(dev_info, "name", name);

    /* Manufacturer and model */
    if (device->manufacturer[0] != '\0') {
        cJSON_AddStringToObject(dev_info, "manufacturer", device->manufacturer);
    }
    if (device->model[0] != '\0') {
        cJSON_AddStringToObject(dev_info, "model", device->model);
    }

    /* Link to bridge device */
    cJSON_AddStringToObject(dev_info, "via_device", "zigbee2mqtt_bridge");
 ...
 */
static esp_err_t publish_action_button(const device_t *device,
                                        const char *action, const char *icon)
{
    ...
    snprintf(name, sizeof(name), "%s %s", get_display_name(device), action_cap);
 ...
 */
static esp_err_t publish_mode_select(const device_t *device)
{
    cJSON *config = cJSON_CreateObject();
    if (config == NULL) {
        return ESP_ERR_NO_MEM;
    ...
    char name[GW_BUFFER_SIZE_SMALL];
    snprintf(name, sizeof(name), "%s Mode", get_display_name(device));
    cJSON_AddStringToObject(config, "name", name);
    cJSON_AddStringToObject(config, "unique_id", unique_id);
 ...
 */
static esp_err_t publish_number_entity(const device_t *device,
                                        ...
                                        const char *unit, const char *icon)
{
    ...
    char name[GW_BUFFER_SIZE_SMALL];
    snprintf(name, sizeof(name), "%s %s", get_display_name(device), display_name);
    cJSON_AddStringToObject(config, "name", name);
    cJSON_AddStringToObject(config, "unique_id", unique_id);
 ...
 */
static esp_err_t publish_switch_entity(const device_t *device,
                                        const char *field, const char *display_name,
                                        const char *icon)
{
    cJSON *config = cJSON_CreateObject();
...
static esp_err_t fingerbot_publish_discovery(const device_t *device)
{
    if (device == NULL) {
    ...
    /* === 6. Sustain Time (Tuya DP unit = 100ms, expose as whole seconds) === */
    ret = publish_number_entity(device, "sustain_time", "Sustain Time",
                                 0, 25, 1, "s", "mdi:timer-outline");
    if (ret != ESP_OK) {
    ...
    /* === 9. Click Control (Switch, Fingerbot Plus only) === */
    ret = publish_switch_entity(device, "click_control", "Click Control",
                                 "mdi:gesture-tap-button");
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Failed to publish click_control: %s", esp_err_to_name(ret));
    }
    ...
    /* === 10. Battery (Sensor) === */
    {
        ...
        if (ret == ESP_OK) {
            ...
            if (config != NULL) {
                char name[GW_BUFFER_SIZE_SMALL];
                snprintf(name, sizeof(name), "%s Battery", get_display_name(device));
                cJSON_AddStringToObject(config, "name", name);
                cJSON_AddStringToObject(config, "unique_id", unique_id);
    ...
    /* === 11. Click Count (Sensor, diagnostic, total_increasing) === */
    {
        char unique_id[MQTT_UNIQUE_ID_MAX_LEN];
        snprintf(unique_id, sizeof(unique_id), "%s_click_count", ieee_str);
        ...
        if (ret == ESP_OK) {
            ...
            if (config != NULL) {
                char name[GW_BUFFER_SIZE_SMALL];
                snprintf(name, sizeof(name), "%s Click Count", get_display_name(device));
                cJSON_AddStringToObject(config, "name", name);
                cJSON_AddStringToObject(config, "unique_id", unique_id);
                ...
                cJSON_AddStringToObject(config, "state_topic", state_topic);
                cJSON_AddStringToObject(config, "value_template",
                                         "{{ value_json.click_count | default(0) }}");

                add_device_info(config, device);
                ret = publish_discovery_config(topic, config, "sensor click_count",
                                                device->friendly_name);
            }
        }
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "Failed to publish click_count sensor: %s", esp_err_to_name(ret));
        }
    }

    /* === 12. Program (Text input) === */
    {
        ...
        if (ret == ESP_OK) {
            ...
            if (config != NULL) {
                char name[GW_BUFFER_SIZE_SMALL];
                snprintf(name, sizeof(name), "%s Program", get_display_name(device));
                cJSON_AddStringToObject(config, "name", name);
    ...
    /* === 16. Test DP (Text, disabled by default) === */
    {
        char uid[GW_BUFFER_SIZE_SMALL];
    ...
    /* === 17. Raw CMD (Text, disabled by default) === */
    {
    ...
    /* === Cleanup: Remove stale light entity if present ===
     * Fingerbot has DEV_CAP_ON_OFF but no DEV_CAP_BRIGHTNESS, so the generic
     * discovery dedup logic correctly publishes a switch (not light). However,
     * a previous firmware version may have published a light entity with
     * retained flag. Clear it by publishing an empty config. */
    {
        char light_uid[80];
        snprintf(light_uid, sizeof(light_uid), "%s_light", ieee_str);

        char light_topic[MQTT_TOPIC_MAX_LEN];
        ret = mqtt_topic_ha_discovery("light", light_uid, light_topic, sizeof(light_topic));
        if (ret == ESP_OK) {
            char *topic_heap = strdup(light_topic);
            char *empty_payload = strdup("");
            if (topic_heap != NULL && empty_payload != NULL) {
                evt_ha_discovery_publish_t evt = {
                    .topic = topic_heap,
