...
bool zb_device_has_dehumid_control(uint16_t short_addr)
{
    ...
    return device_zigbee_has_cluster(device, ZB_ZCL_CLUSTER_ID_DEHUMIDIFICATION_CONTROL);
}
...
esp_err_t zb_dehumid_read_state(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Dehumidification read request queued, TSN=0x%02X", tsn);
                                   ...
                                   uint8_t setpoint)
{
    ...
    ESP_LOGD(TAG, "Dehumidification setpoint write queued, TSN=0x%02X", tsn);
                                     ...
                                     uint8_t hysteresis)
{
    ...
    ESP_LOGD(TAG, "Dehumidification hysteresis write queued, TSN=0x%02X", tsn);
                                   ...
                                   uint8_t max_cool)
{
    ...
    ESP_LOGD(TAG, "Dehumidification max cool write queued, TSN=0x%02X", tsn);
