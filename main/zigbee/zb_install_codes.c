...
/* ESP-IDF v6.0+ uses PSA Crypto API, v5.x uses legacy mbedTLS MD API */
                                       ...
                                       uint8_t *out_key)
{
    ...
    /* Set up key attributes for HMAC-SHA256 */
    psa_set_key_usage_flags(&attributes, PSA_KEY_USAGE_SIGN_MESSAGE);
    psa_set_key_algorithm(&attributes, PSA_ALG_HMAC(PSA_ALG_SHA_256));
    psa_set_key_type(&attributes, PSA_KEY_TYPE_HMAC);
    psa_set_key_bits(&attributes, HMAC_KEY_LEN * 8);
    ...
    status = psa_mac_compute(key_id, PSA_ALG_HMAC(PSA_ALG_SHA_256),
                             install_code, code_len,
                             hmac_output, sizeof(hmac_output), &output_len);

    /* Always destroy the key after use */
...
#else
    /* ESP-IDF v5.x uses legacy mbedTLS MD API */
...
esp_err_t zb_install_codes_init(void)
{
    ...
    if (s_install_codes == NULL) {
        s_install_codes = mem_ng_calloc(ZB_INSTALL_CODE_MAX_ENTRIES,
                                         sizeof(zb_install_code_entry_t),
                                         MEM_CAP_PSRAM);
        if (!s_install_codes) {
