...
static zb_electrical_state_t* get_or_create_electrical_state(uint16_t short_addr)
{
    ...
    s_electrical_states[idx].voltage_divisor = ZB_ELECTRICAL_VOLTAGE_DIVISOR;    /* V * 10 */
    ...
    s_electrical_states[idx].power_divisor = ZB_ELECTRICAL_POWER_DIVISOR;         /* W * 10 */
...
esp_err_t zb_cluster_electrical_init(void)
{
    ...
    if (s_electrical_states == NULL) {
        s_electrical_states = mem_ng_calloc(ZB_STATE_MAX_ELECTRICAL,
    ...
    if (s_electrical_addrs == NULL) {
        s_electrical_addrs = mem_ng_calloc(ZB_STATE_MAX_ELECTRICAL,
                                           sizeof(uint16_t),
                                           MEM_CAP_PSRAM);
    ...
    if (!s_electrical_states || !s_electrical_addrs) {
        ...
        mem_ng_free(s_electrical_states);
        s_electrical_states = NULL;
        mem_ng_free(s_electrical_addrs);
    ...
    if (s_metering_states == NULL) {
        s_metering_states = mem_ng_calloc(ZB_STATE_MAX_METERING,
    ...
    if (s_metering_addrs == NULL) {
        s_metering_addrs = mem_ng_calloc(ZB_STATE_MAX_METERING,
                                         sizeof(uint16_t),
                                         MEM_CAP_PSRAM);
        if (!s_metering_addrs) {
            s_metering_addrs = mem_ng_calloc(ZB_STATE_MAX_METERING,
    ...
    if (!s_metering_states || !s_metering_addrs) {
        ...
        mem_ng_free(s_electrical_addrs);
        s_electrical_addrs = NULL;
        mem_ng_free(s_metering_states);
        s_metering_states = NULL;
        mem_ng_free(s_metering_addrs);
...
esp_err_t zb_electrical_read_values(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Electrical measurement read request queued, TSN=0x%02X", tsn);
...
esp_err_t zb_electrical_read_scaling(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Electrical scaling read request queued, TSN=0x%02X", tsn);
...
esp_err_t zb_metering_read_values(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Metering read request queued, TSN=0x%02X", tsn);
