...
esp_err_t zb_touchlink_init(void)
{
    ...
    if (s_scan_results == NULL) {
        s_scan_results = mem_ng_calloc(ZB_TOUCHLINK_MAX_SCAN_RESULTS,
        ...
        if (!s_scan_results) {
            ...
            xTimerDelete(s_scan_timer, GW_TIMEOUT_MEDIUM_TICKS);
            s_scan_timer = NULL;
            vSemaphoreDelete(s_touchlink_mutex);
            s_touchlink_mutex = NULL;
            return ESP_ERR_NO_MEM;
                            ...
                            void *user_data)
{
    ...
    xSemaphoreTake(s_touchlink_mutex, GW_TIMEOUT_LONG_TICKS);
...
esp_err_t zb_touchlink_publish_scan_results(void)
{
    ...
    ESP_LOGI(TAG, "Publishing Touchlink scan results via event bus...");

    /* Publish scan complete event - mqtt_event_handler.c will handle MQTT publishing
     * The handler can call zb_touchlink_get_scan_results() to get the actual results */
    evt_zb_touchlink_result_t evt = {
        .ieee_addr = 0,  /* Scan results, not a specific device */
        .short_addr = 0,
        .success = (s_scan_status == ZB_TOUCHLINK_SCAN_STATUS_COMPLETE),
        .error_code = 0
    };

    esp_err_t ret = event_publish(EVT_ZB_TOUCHLINK_RESULT, &evt, sizeof(evt));

    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to publish scan results event: %s", esp_err_to_name(ret));
    } else {
        ESP_LOGI(TAG, "Scan results event published successfully (%zu devices)",
                 s_scan_result_count);
    }
