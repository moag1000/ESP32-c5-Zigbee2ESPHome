...
esp_err_t zb_diagnostics_get_coordinator(zb_diagnostics_t *diag)
{
    ...
    xSemaphoreTake(s_diag_mutex, GW_TIMEOUT_LONG_TICKS);
                                      ...
                                      size_t max_count)
{
    ...
    for (size_t i = 0; i < device_count && copied < max_count; i++) {
        ...
        dev_diag->short_addr = dev->proto.zigbee.short_addr;
        /* Copy IEEE address from device_t id (uint64) to array */
        memcpy(dev_diag->ieee_addr, &dev->id, 8);
        dev_diag->last_updated = dev->last_seen;
        dev_diag->valid = (dev->availability == DEV_AVAIL_ONLINE);
...
esp_err_t zb_diagnostics_publish_mqtt(void)
{
    ...
    /* Merge state into device registry before publishing event */
    esp_err_t merge_ret = device_registry_merge_state(0, root);
    cJSON_Delete(root);

    if (merge_ret != ESP_OK) {
        ESP_LOGW(TAG, "Failed to merge diagnostics state: %s", esp_err_to_name(merge_ret));
    }
    ...
    evt_device_state_t evt = {
        .ieee_addr = 0,  /* Bridge/coordinator */
        .short_addr = 0x0000,  /* Coordinator address */
        .endpoint = 0,
        .cluster_id = 0,
        .attr_id = 0,
        .json_state = NULL,
    ...
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Diagnostics published via event bus");
    } else {
        ESP_LOGE(TAG, "Failed to publish diagnostics event: %s", esp_err_to_name(ret));
    }
...
bool zb_diagnostics_has_device_temp_cluster(uint16_t short_addr)
{
    ...
    /* Get device information from NG registry */
    ...
    return device_zigbee_has_cluster(device, ZB_ZCL_CLUSTER_ID_DEVICE_TEMP_CONFIG);
}
