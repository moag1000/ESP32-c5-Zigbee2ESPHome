...
static bool s_closures_initialized = false;
                                        ...
                                        uint8_t cmd_id, bool is_moving)
{
    ...
    ESP_LOGD(TAG, "Window covering command 0x%02X to 0x%04X EP%d queued, TSN=0x%02X",
             cmd_id, short_addr, endpoint, tsn);
...
esp_err_t zb_cluster_closures_init(void)
{
    ...
    if (s_window_covering_states == NULL) {
        s_window_covering_states = mem_ng_calloc(ZB_STATE_MAX_WINDOW_COVERING,
    ...
    if (s_window_covering_addrs == NULL) {
        s_window_covering_addrs = mem_ng_calloc(ZB_STATE_MAX_WINDOW_COVERING,
                                                sizeof(uint16_t),
                                                MEM_CAP_PSRAM);
    ...
    if (!s_window_covering_states || !s_window_covering_addrs) {
        ...
        mem_ng_free(s_window_covering_states);
        s_window_covering_states = NULL;
        mem_ng_free(s_window_covering_addrs);
        s_window_covering_addrs = NULL;
        return ESP_ERR_NO_MEM;
    }

    s_closures_initialized = true;
...
esp_err_t zb_window_covering_read_position(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Window covering read position request queued, TSN=0x%02X", tsn);
                                                    ...
                                                    uint8_t percentage)
{
    ...
    ESP_LOGD(TAG, "GoToLiftPercentage command queued, TSN=0x%02X", tsn);
                                                    ...
                                                    uint8_t percentage)
{
    ...
    if (percentage > ZB_WINDOW_COVERING_PERCENT_MAX) {
        ESP_LOGW(TAG, "Clamping tilt percentage %d to %d", percentage, ZB_WINDOW_COVERING_PERCENT_MAX);
        percentage = ZB_WINDOW_COVERING_PERCENT_MAX;
    }

    ESP_LOGI(TAG, "Sending GoToTiltPercentage(%d%%) to 0x%04X EP%d",
             percentage, short_addr, endpoint);
    ...
    esp_zb_zcl_custom_cluster_cmd_t cmd_req = {
        ...
        .cluster_id = ZB_ZCL_CLUSTER_ID_WINDOW_COVERING,
        .direction = ESP_ZB_ZCL_CMD_DIRECTION_TO_SRV,
        .custom_cmd_id = ZB_ZCL_CMD_WINDOW_COVERING_GO_TO_TILT_PERCENT,
        .data = {
            .type = ESP_ZB_ZCL_ATTR_TYPE_U8,
            .size = 1,
            .value = &payload,
    ...
    ESP_LOGD(TAG, "GoToTiltPercentage command queued, TSN=0x%02X", tsn);

    zb_window_covering_state_t *tilt_state = get_or_create_window_covering_state(short_addr);
    if (tilt_state) {
        tilt_state->is_moving = true;
    }
...
{
    if (!s_closures_initialized || value == NULL) {
