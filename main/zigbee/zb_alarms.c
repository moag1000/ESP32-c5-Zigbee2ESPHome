...
{
    /* Get device friendly name from NG registry */
    ...
    /* Publish via Event Bus - mqtt_event_handler will forward to MQTT */
    evt_zb_alarm_t evt = {
        .ieee_addr = device ? device->id : 0,
        .short_addr = short_addr,
        .endpoint = 1,
        .cluster_id = cluster_id,
        .alarm_code = alarm_code,
        .description = zb_alarms_code_to_string(alarm_code, cluster_id),
    };
    event_publish(EVT_ZB_ALARM_TRIGGERED, &evt, sizeof(evt));
}
...
esp_err_t zb_alarms_init(void)
{
    ...
    /* Allocate alarm storage using memory manager (auto-selects PSRAM) */
    if (s_device_alarms == NULL) {
        s_device_alarms = mem_ng_calloc(ZB_ALARMS_MAX_DEVICES, sizeof(zb_device_alarms_t), MEM_CAP_PSRAM);
        if (!s_device_alarms) {
                                     ...
                                     uint8_t alarm_code, uint16_t cluster_id)
{
    ...
    ESP_LOGI(TAG, "Reset Alarm command queued (TSN=%u)", tsn);
...
esp_err_t zb_alarms_cmd_reset_all(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGI(TAG, "Reset All Alarms command queued (TSN=%u)", tsn);
...
esp_err_t zb_alarms_cmd_get_alarm(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGI(TAG, "Get Alarm command queued (TSN=%u)", tsn);
...
esp_err_t zb_alarms_cmd_reset_alarm_log(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGI(TAG, "Reset Alarm Log command queued (TSN=%u)", tsn);
