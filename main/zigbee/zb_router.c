...
#define ZB_ROUTER_STACK_SIZE        (4 * 1024)
...
esp_err_t zb_router_get_parent_info(zb_router_parent_info_t *parent_info)
{
    ...
    xSemaphoreTake(s_router_mutex, GW_TIMEOUT_LONG_TICKS);
...
static void set_router_state(zb_router_state_t state)
{
    ...
    /* Publish state change via event bus */
...
static void handle_network_left(void)
{
    ...
    /* Publish offline status via event bus */
    publish_router_state_mqtt();
}
#endif /* CONFIG_ZIGBEE_DEVICE_TYPE_ROUTER */
...
static esp_err_t publish_router_state_mqtt(void)
{
    ...
    evt_zb_router_status_t evt = {
        .ieee_addr = 0,  /* Will be set if needed */
        .short_addr = zb_router_is_joined() ? esp_zb_get_short_address() : 0,
        .online = zb_router_is_joined(),
        .lqi = s_has_parent_info ? s_parent_info.lqi : 0,
        .rssi = s_has_parent_info ? s_parent_info.rssi : 0,
        .json_parent = NULL,
        .json_neighbors = NULL,
    };
    event_publish(EVT_ZB_ROUTER_STATUS, &evt, sizeof(evt));
...
static esp_err_t publish_parent_info_mqtt(void)
{
    ...
    evt_zb_router_status_t evt = {
        ...
        .short_addr = zb_router_is_joined() ? esp_zb_get_short_address() : 0,
        .online = zb_router_is_joined(),
        .lqi = s_parent_info.lqi,
        .rssi = s_parent_info.rssi,
        .json_parent = json_buf,
        .json_neighbors = NULL,
    };
    event_publish(EVT_ZB_ROUTER_STATUS, &evt, sizeof(evt));
