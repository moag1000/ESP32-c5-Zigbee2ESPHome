...
esp_err_t zb_cluster_security_init(void)
{
    ...
    if (s_door_lock_states == NULL) {
        s_door_lock_states = mem_ng_calloc(ZB_STATE_MAX_DOOR_LOCK,
                                           sizeof(zb_door_lock_state_struct_t),
                                           MEM_CAP_PSRAM);
        if (!s_door_lock_states) {
            s_door_lock_states = mem_ng_calloc(ZB_STATE_MAX_DOOR_LOCK,
                                               sizeof(zb_door_lock_state_struct_t),
                                               MEM_CAP_DEFAULT);  /* Fallback */
        }
    }
    if (s_door_lock_addrs == NULL) {
        s_door_lock_addrs = mem_ng_calloc(ZB_STATE_MAX_DOOR_LOCK,
                                          sizeof(uint16_t),
                                          MEM_CAP_PSRAM);
    ...
    if (!s_door_lock_states || !s_door_lock_addrs) {
        ...
        mem_ng_free(s_door_lock_states);
        s_door_lock_states = NULL;
        mem_ng_free(s_door_lock_addrs);
    ...
    if (s_ias_zone_states == NULL) {
        s_ias_zone_states = mem_ng_calloc(ZB_STATE_MAX_IAS_ZONE,
    ...
    if (s_ias_zone_addrs == NULL) {
        s_ias_zone_addrs = mem_ng_calloc(ZB_STATE_MAX_IAS_ZONE,
                                         sizeof(uint16_t),
                                         MEM_CAP_PSRAM);
        if (!s_ias_zone_addrs) {
            s_ias_zone_addrs = mem_ng_calloc(ZB_STATE_MAX_IAS_ZONE,
    ...
    if (!s_ias_zone_states || !s_ias_zone_addrs) {
        ...
        mem_ng_free(s_door_lock_addrs);
        s_door_lock_addrs = NULL;
        mem_ng_free(s_ias_zone_states);
        s_ias_zone_states = NULL;
        mem_ng_free(s_ias_zone_addrs);
...
esp_err_t zb_door_lock_read_state(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Door lock read request queued, TSN=0x%02X", tsn);
...
esp_err_t zb_door_lock_cmd_lock(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Lock command queued, TSN=0x%02X", tsn);
...
esp_err_t zb_door_lock_cmd_unlock(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Unlock command queued, TSN=0x%02X", tsn);
...
esp_err_t zb_ias_zone_read_state(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "IAS Zone read request queued, TSN=0x%02X", tsn);
...
esp_err_t zb_ias_zone_write_cie_address(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "CIE address write queued, TSN=0x%02X", tsn);
                                       ...
                                       uint8_t zone_id)
{
    ...
    ESP_LOGD(TAG, "Zone Enroll Response queued, TSN=0x%02X", tsn);

    if (response_code == ZB_IAS_ZONE_ENROLL_SUCCESS) {
        /* Update local state */
    ...
    switch (attr_id) {
        ...
        case ZB_ZCL_ATTR_IAS_ZONE_TYPE_ID:
            if (value_len >= 2) {
                ...
                /* Update device capabilities based on zone type using NG device registry */
                device_t *device = device_registry_get_by_short_addr(short_addr);
                if (device != NULL) {
                    switch (state->zone_type) {
                        case ZB_IAS_ZONE_TYPE_MOTION_SENSOR:
                            device_set_capability(device, DEV_CAP_MOTION);
                            break;
                        case ZB_IAS_ZONE_TYPE_CONTACT_SWITCH:
                            device_set_capability(device, DEV_CAP_CONTACT);
                            break;
                        case ZB_IAS_ZONE_TYPE_FIRE_SENSOR:
                            device_set_capability(device, DEV_CAP_SMOKE);
                            break;
                        case ZB_IAS_ZONE_TYPE_WATER_SENSOR:
                            device_set_capability(device, DEV_CAP_WATER_LEAK);
                            break;
                        case ZB_IAS_ZONE_TYPE_GAS_SENSOR:
                        case ZB_IAS_ZONE_TYPE_CARBON_MONOXIDE:
                            device_set_capability(device, DEV_CAP_CO);
                            break;
                        case ZB_IAS_ZONE_TYPE_VIBRATION_SENSOR:
                            device_set_capability(device, DEV_CAP_VIBRATION);
                            break;
                        default:
                            /* Generic IAS zone - no specific capability */
                            break;
                    }
                } else {
                    ESP_LOGW(TAG, "Device 0x%04X not found in registry, skipping capability update", short_addr);
                }
            }
            break;
