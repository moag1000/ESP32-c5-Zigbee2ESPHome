...
esp_err_t zb_cluster_hvac_init(void)
{
    ...
    /* Allocate Thermostat arrays using memory manager */
    if (s_thermostat_states == NULL) {
        s_thermostat_states = mem_ng_calloc(ZB_STATE_MAX_THERMOSTAT,
                                            sizeof(zb_thermostat_state_t), MEM_CAP_PSRAM);
    }
    if (s_thermostat_addrs == NULL) {
        s_thermostat_addrs = mem_ng_calloc(ZB_STATE_MAX_THERMOSTAT,
                                           sizeof(uint16_t), MEM_CAP_PSRAM);
    }

    if (!s_thermostat_states || !s_thermostat_addrs) {
        ESP_LOGE(TAG, "Failed to allocate thermostat state arrays");
        mem_ng_free(s_thermostat_states);
        s_thermostat_states = NULL;
        mem_ng_free(s_thermostat_addrs);
    ...
    /* Allocate Fan Control arrays using memory manager */
    if (s_fan_control_states == NULL) {
        s_fan_control_states = mem_ng_calloc(ZB_STATE_MAX_FAN_CONTROL,
                                             sizeof(zb_fan_control_state_t), MEM_CAP_PSRAM);
    }
    if (s_fan_control_addrs == NULL) {
        s_fan_control_addrs = mem_ng_calloc(ZB_STATE_MAX_FAN_CONTROL,
                                            sizeof(uint16_t), MEM_CAP_PSRAM);
    }
    ...
    if (!s_fan_control_states || !s_fan_control_addrs) {
        ...
        mem_ng_free(s_thermostat_addrs);
        s_thermostat_addrs = NULL;
        mem_ng_free(s_fan_control_states);
        s_fan_control_states = NULL;
        mem_ng_free(s_fan_control_addrs);
...
esp_err_t zb_thermostat_read_state(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Thermostat read request queued, TSN=0x%02X", tsn);
                                              ...
                                              int16_t temperature)
{
    ...
    ESP_LOGD(TAG, "Heating setpoint write queued, TSN=0x%02X", tsn);
                                              ...
                                              int16_t temperature)
{
    ...
    ESP_LOGD(TAG, "Cooling setpoint write queued, TSN=0x%02X", tsn);
                                         ...
                                         zb_thermostat_system_mode_t mode)
{
    ...
    ESP_LOGD(TAG, "System mode write queued, TSN=0x%02X", tsn);
...
esp_err_t zb_fan_control_read_state(uint16_t short_addr, uint8_t endpoint)
{
    ...
    ESP_LOGD(TAG, "Fan control read request queued, TSN=0x%02X", tsn);
                                   ...
                                   zb_fan_mode_t mode)
{
    ...
    ESP_LOGD(TAG, "Fan mode write queued, TSN=0x%02X", tsn);
