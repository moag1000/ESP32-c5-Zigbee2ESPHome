...
/**
 * @note Per-device availability metadata is stored in device_t.proto.zigbee.avail_meta
 *       (type device_avail_meta_t, defined in unified_device.h).
 *       The parallel zb_availability_device_t array was eliminated in AP-7.9.
 */
...
/**
 * @brief Add a device to availability tracking with explicit initial state
 *
 * Like zb_availability_add_device() but allows setting the initial state
 * (e.g., ZB_AVAIL_UNKNOWN for boot-restored devices that haven't communicated yet).
 *
 * @param[in] short_addr Device short address
 * @param[in] power_type Device power source type
 * @param[in] initial_state Initial availability state
 * @return ESP_OK on success
 * @return ESP_ERR_NO_MEM if device list is full
 * @return ESP_ERR_INVALID_ARG if address is invalid
 */
