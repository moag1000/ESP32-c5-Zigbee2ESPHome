...
esp_err_t zb_reporting_init(void)
{
    ...
    /* Allocate entry storage using memory manager (auto-selects PSRAM for large allocs) */
    s_entries = mem_ng_calloc(ZB_REPORTING_MAX_ENTRIES, sizeof(zb_reporting_entry_t), MEM_CAP_PSRAM);
    if (s_entries == NULL) {
                                  ...
                                  zb_reporting_config_t *config)
{
    ...
    uint64_t device_ieee = device->id;
                            ...
                            uint16_t cluster_id, uint16_t attr_id)
{
    ...
    (void)device; /* Used only for validation */
...
esp_err_t zb_reporting_set_defaults(uint16_t short_addr)
{
    ...
    /* Get device info from NG registry */
    device_t *device = device_registry_get_by_short_addr(short_addr);
    if (device == NULL) {
        ESP_LOGE(TAG, "Device 0x%04X not found", short_addr);
        return ESP_ERR_NOT_FOUND;
    ...
    for (const zb_reporting_default_t *def = s_defaults; def->cluster_name != NULL; def++) {
        ...
        bool has_cluster = device_zigbee_has_cluster(device, def->cluster_id);
        ...
        if (has_cluster) {
            ...
            esp_err_t ret = zb_reporting_configure(short_addr, device->proto.zigbee.endpoint, &config);
            if (ret == ESP_OK) {
                configured++;
                                                   ...
                                                   uint16_t attr_id)
{
    ...
    /* Get device info for response from NG registry */
    device_t *device = device_registry_get_by_short_addr(short_addr);
    ...
    /* Notify event */
    uint64_t device_ieee = (device != NULL) ? device->id : 0;
                                             ...
                                             uint16_t reportable_change)
{
    ...
    /* Get device info from NG registry */
    device_t *rsp_device = device_registry_get_by_short_addr(short_addr);
    char device_name[ZB_REPORTING_NAME_MAX_LEN];
    if (rsp_device != NULL && strlen(rsp_device->friendly_name) > 0) {
        strncpy(device_name, rsp_device->friendly_name, sizeof(device_name) - 1);
    } else {
        snprintf(device_name, sizeof(device_name), "0x%04X", short_addr);
    }
    ...
    /* Notify event */
    uint64_t device_ieee = (rsp_device != NULL) ? rsp_device->id : 0;
...
esp_err_t zb_reporting_publish_configure_response(const zb_reporting_result_t *result)
{
    ...
    /* Resolve device IEEE address from device name */
    uint16_t short_addr = 0;
    uint8_t default_ep = 1;
    uint64_t ieee_addr = resolve_device_ieee(result->device_name, &short_addr, &default_ep);

    /* Build and publish event */
    evt_zb_reporting_response_t evt = {
        .ieee_addr = ieee_addr,
        .short_addr = short_addr,
        .cluster_id = result->config.cluster_id,
        .attribute_id = result->config.attr_id,
        .min_interval = result->config.min_interval,
        .max_interval = result->config.max_interval,
        .success = result->success,
        .error_code = (uint8_t)result->status,
    };

    ESP_LOGI(TAG, "Publishing configure response event: cluster=0x%04X attr=0x%04X success=%d",
             evt.cluster_id, evt.attribute_id, evt.success);

    return event_publish(EVT_ZB_REPORTING_RESPONSE, &evt, sizeof(evt));
}
...
esp_err_t zb_reporting_publish_read_response(const zb_reporting_result_t *result)
{
    ...
    ESP_LOGI(TAG, "Publishing read response event: cluster=0x%04X attr=0x%04X success=%d",
             evt.cluster_id, evt.attribute_id, evt.success);
                                     ...
                                     uint8_t *default_endpoint)
{
    ...
    *default_endpoint = 1;
    ...
    /* Use NG registry's unified lookup: handles both IEEE strings and friendly names */
    device_t *device = device_registry_find_by_id(device_str);
    if (device != NULL && device->protocol == DEV_PROTOCOL_ZIGBEE) {
        *short_addr = device->proto.zigbee.short_addr;
        *default_endpoint = device->proto.zigbee.endpoint;
        return device->id;
    }
