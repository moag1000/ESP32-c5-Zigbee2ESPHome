...
esp_err_t zb_topology_scan(zb_topology_scan_cb_t callback)
{
    ...
    for (size_t i = 0; i < device_count && s_pending_lqi_count < ZB_TOPOLOGY_MAX_NODES; i++) {
        ...
        if (node != NULL && node->device_type == ZB_TOPO_DEVICE_TYPE_ROUTER) {
            s_pending_lqi[s_pending_lqi_count].short_addr = dev->proto.zigbee.short_addr;
...
esp_err_t zb_topology_publish_mqtt(void)
{
    ...
    /* Convert IEEE address to uint64_t (little-endian) */
    uint64_t coord_ieee_u64 = 0;
    for (int i = 0; i < 8; i++) {
        coord_ieee_u64 |= ((uint64_t)coord_ieee[i]) << (i * 8);
    }
    ...
    esp_err_t ret = event_publish(EVT_ZB_TOPOLOGY_RESPONSE, &evt, sizeof(evt));

    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Published network topology event (%zu bytes)", strlen(json_str));
    } else {
        ESP_LOGE(TAG, "Failed to publish topology event: %s", esp_err_to_name(ret));
    }
...
esp_err_t zb_topology_save_nvs(void)
{
    ...
    topo_cache_t *cache = mem_alloc(sizeof(topo_cache_t), MEM_CAP_DEFAULT);
    if (cache == NULL) {
        xSemaphoreGive(s_mutex);
        nvs_close(handle);
        return ESP_ERR_NO_MEM;
...
esp_err_t zb_topology_load_nvs(void)
{
    ...
    topo_cache_t *cache = mem_alloc(size, MEM_CAP_DEFAULT);
    if (cache == NULL) {
        nvs_close(handle);
        return ESP_ERR_NO_MEM;
    }
