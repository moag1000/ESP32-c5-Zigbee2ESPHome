...
/* IEEE 802.15.4 coexistence priority control */
...
/* Note: BLE lifecycle is now managed by lifecycle_manager.c */
 ...
 * and caused sleepy devices (e.g. Aqara DJT11LM) to timeout during join. */
#define ZB_COORDINATOR_PRIORITY     20
...
/**
 * @brief Iterator callback to register restored devices in availability tracker
 *
 * Called via device_registry_iterate_zigbee() during coordinator init.
 * Note: device_registry holds the mutex during iteration, so we must NOT
 * call device_registry functions here. AP-7.9: avail_meta now lives in
 * device_t, so we set it directly on the dev pointer we already have.
 */
static bool register_restored_device_avail(device_t *dev, void *ctx)
{
    (void)ctx;
    if (dev->proto.zigbee.short_addr == 0 ||
        dev->proto.zigbee.short_addr == 0xFFFF) {
        return true; /* Skip invalid, continue */
    }
    /* Use persisted power info for correct availability timeouts */
    uint8_t power = (uint8_t)ZB_AVAIL_POWER_UNKNOWN;
    if (dev->proto.zigbee.power_info.power_info_valid) {
        if (dev->proto.zigbee.power_info.current_power_source & 0x04) {
            power = (uint8_t)ZB_AVAIL_POWER_BATTERY;
        } else if (dev->proto.zigbee.power_info.current_power_source & 0x01) {
            power = (uint8_t)ZB_AVAIL_POWER_MAINS;
        }
    }
    /* Boot-restored devices start as ONLINE (optimistic, like zigbee2mqtt).
     * The availability tracker will mark them OFFLINE if they don't communicate
     * within their timeout window. Battery devices get generous timeouts.
     * Set avail_meta directly (cannot call zb_availability_add_device()
     * from within device_registry_iterate — would deadlock on registry mutex). */
    device_avail_meta_t *meta = &dev->proto.zigbee.avail_meta;
    meta->power_type = power;
    meta->backoff_multiplier = 1;  /* 1 = tracked */
    meta->consecutive_failures = 0;
    meta->custom_timeout = 0;
    meta->pending_response = false;
    meta->last_check = 0;
    meta->next_check = 0;  /* Will be calculated on first task loop */
    dev->availability = DEV_AVAIL_ONLINE;

    ESP_LOGI("ZB_COORD", "Registered restored device 0x%04X (power=%d, state=ONLINE) in availability tracker",
             dev->proto.zigbee.short_addr, power);
...
esp_err_t zb_coordinator_init(void)
{
    ...
    ret = zb_cluster_multistate_init();
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Multistate cluster init failed: %s", esp_err_to_name(ret));
        goto cleanup;
    }

    /* NG device_persistence_load_all() already called in foundation_init.c.
     * Count loaded devices from NG registry for availability tracker. */
    size_t loaded_devices = device_registry_count();
    ...
    if (loaded_devices > 0 && avail_ret == ESP_OK) {
        size_t registered = device_registry_iterate_zigbee(
            register_restored_device_avail, NULL);
        ESP_LOGI(TAG, "Registered %zu restored devices in availability tracker",
                 registered);
    }
    ...
    /* Disable TCLK (Trust Center Link Key) exchange requirement.
     * Some devices (like Aqara DJT11LM) don't properly implement the standard
     * TCLK exchange protocol and will timeout during authorization.
     * Disabling this requirement allows these devices to join using the
    ...
    /* Set primary network channel (use specific channel from config instead of all channels) */
...
bool zb_coordinator_is_running(void)
{
    ...
    xSemaphoreTake(s_coordinator_mutex, GW_TIMEOUT_LONG_TICKS);
...
esp_err_t zb_coordinator_permit_join_with_options(const zb_permit_join_options_t *options)
{
    ...
    /* Block permit_join during BOOT phase - services not ready */
    lifecycle_phase_t phase = lifecycle_get_phase();
    if (phase == LIFECYCLE_PHASE_BOOT) {
        ESP_LOGW(TAG, "Rejecting permit_join during BOOT phase - services not ready");
        return ESP_ERR_INVALID_STATE;
    }
    ...
    if (duration == 0) {
        ...
        if (ret == ESP_OK) {
            ...
            /* Coex priority was already MIDDLE during permit_join, keep it there */

            /* Restore WiFi to performance mode */
            wifi_manager_set_power_mode(WIFI_POWER_MODE_PERFORMANCE);
            ESP_LOGI(TAG, "WiFi power mode restored to PERFORMANCE");

            /* Return to NORMAL phase - restarts all paused services */
            lifecycle_enter_phase(LIFECYCLE_PHASE_NORMAL);

            /* Update LED status */
            led_status_manager_set_condition(LED_COND_PERMIT_JOIN, false);

            /* Publish permit join ended event to bridge and event bus */
            bridge_event_permit_join(false, 0);
            {
                evt_permit_join_t evt = { .enabled = false, .duration = 0 };
                event_publish(EVT_ZB_PERMIT_JOIN, &evt, sizeof(evt));
            }
        ...
        if (ret == ESP_OK) {
            ...
            /* CRITICAL: Enter PAIRING phase to free maximum internal RAM.
             * This stops non-essential services (BLE, ESPHome, state persistence,
             * system monitor) to give Zigbee stack maximum resources for the
             * timing-critical TCLK key exchange during device association. */
            lifecycle_enter_phase(LIFECYCLE_PHASE_PAIRING);

            /* Set IEEE 802.15.4 coexistence priority to MIDDLE (not HIGH).
             *
             * HIGH priority starved WiFi and caused BEACON_TIMEOUT disconnects.
             * MIDDLE priority gives Zigbee some advantage without killing WiFi.
             * This should keep WiFi connected during device pairing while still
...
#if CONFIG_ESP_COEX_SW_COEXIST_ENABLE
            {
                ...
                esp_zb_lock_acquire(GW_TIMEOUT_VERY_LONG_TICKS);
                esp_ieee802154_set_coex_config(coex_mid);
                esp_zb_lock_release();
                ESP_LOGI(TAG, "IEEE 802.15.4 coex priority set to MIDDLE for permit_join");
            }
#endif
            ...
            /* Publish permit join started event to bridge and event bus */
            bridge_event_permit_join(true, duration);
            {
                evt_permit_join_t evt = { .enabled = true, .duration = duration_sec };
                event_publish(EVT_ZB_PERMIT_JOIN, &evt, sizeof(evt));
            }
...
static void permit_join_timer_callback(void *arg)
{
    ...
    /* Restore WiFi to performance mode */
    wifi_manager_set_power_mode(WIFI_POWER_MODE_PERFORMANCE);
    ESP_LOGI(TAG, "WiFi power mode restored to PERFORMANCE (timer expired)");
...
static void permit_join_update_callback(void *arg)
{
    ...
    ESP_LOGI(TAG, "Permit join remaining: %d seconds", remaining);
...
esp_err_t zb_coordinator_health_check(zb_coordinator_health_t *health)
{
    ...
    /* Device count */
    health->joined_devices = device_registry_count();
