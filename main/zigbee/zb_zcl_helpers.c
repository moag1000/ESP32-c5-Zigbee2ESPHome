                                           ...
                                           bool on)
{
    ...
    ESP_LOGD(TAG, "Sent on/off command to 0x%04X EP%d: %s (TSN=%d)",
             short_addr, endpoint, on ? "ON" : "OFF", tsn);
    /* Record Zigbee message sent for performance metrics */
    perf_metrics_record(PERF_METRIC_ZB_MSG_SENT, 1);
                                          ...
                                          uint8_t level, uint16_t transition_time)
{
    ...
    ESP_LOGD(TAG, "Sent level command to 0x%04X EP%d: level=%d, time=%d (TSN=%d)",
             short_addr, endpoint, level, transition_time, tsn);
    /* Record Zigbee message sent for performance metrics */
    perf_metrics_record(PERF_METRIC_ZB_MSG_SENT, 1);
                                          ...
                                          uint16_t transition_time)
{
    ...
    ESP_LOGD(TAG, "Sent color command to 0x%04X EP%d: x=%d, y=%d, time=%d (TSN=%d)",
             short_addr, endpoint, color_x, color_y, transition_time, tsn);
    /* Record Zigbee message sent for performance metrics */
    perf_metrics_record(PERF_METRIC_ZB_MSG_SENT, 1);
                                     ...
                                     uint16_t attr_count)
{
    ...
    ESP_LOGD(TAG, "Sent read attr command to 0x%04X EP%d cluster=0x%04X attrs=%d (TSN=%d)",
             short_addr, endpoint, cluster_id, attr_count, tsn);
    /* Record Zigbee message sent for performance metrics */
    perf_metrics_record(PERF_METRIC_ZB_MSG_SENT, 1);
                                      ...
                                      uint16_t attr_count)
{
    ...
    ESP_LOGD(TAG, "Sent write attr command to 0x%04X EP%d cluster=0x%04X attrs=%d (TSN=%d)",
             short_addr, endpoint, cluster_id, attr_count, tsn);
    /* Record Zigbee message sent for performance metrics */
    perf_metrics_record(PERF_METRIC_ZB_MSG_SENT, 1);
                                    ...
                                    void *payload)
{
    ...
    /* Acquire Zigbee lock before API call */
    esp_zb_lock_acquire(GW_TIMEOUT_VERY_LONG_TICKS);
    uint8_t tsn = esp_zb_zcl_custom_cluster_cmd_req((esp_zb_zcl_custom_cluster_cmd_t *)payload);
    esp_zb_lock_release();

    ESP_LOGD(TAG, "Sent custom ZCL command cluster=0x%04X cmd=0x%02X (TSN=%d)",
             cluster_id, cmd_id, tsn);
    /* Record Zigbee message sent for performance metrics */
    perf_metrics_record(PERF_METRIC_ZB_MSG_SENT, 1);
