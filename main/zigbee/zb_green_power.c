...
/**
 * @brief Build JSON state string from GP frame
 *
 * @param[in] device    GP device
 * @param[in] frame     GP frame
 * @param[out] json     Output buffer
 * @param[in] json_size Size of output buffer
 * @return Length of JSON string or -1 on error
 */
static int gp_build_state_json(const zb_gp_device_t *device, const zb_gp_frame_t *frame,
                               char *json, size_t json_size)
{
    (void)device;  /* May be used for extended state in future */
...
/**
 * @brief Get IEEE address for GP device (synthesized from source ID or actual IEEE)
 *
 * For Source ID devices, we create a synthetic IEEE address using the GP prefix
 * 0x00000000 + source_id to make it distinguishable.
 *
 * @param[in] gp_addr   GP address
 * @return Synthesized or actual IEEE address
 */
static uint64_t gp_get_ieee_addr(const zb_gp_addr_t *gp_addr)
{
    if (gp_addr->app_id == ZB_GP_APP_ID_SRC_ID) {
        /* Synthesize IEEE from Source ID with GP marker (high bytes 0x00GP) */
        return ((uint64_t)0x0000 << 48) | ((uint64_t)0x4750 << 32) | gp_addr->src_id;
    } else {
        /* IEEE addressing - reconstruct from array */
        uint64_t ieee = 0;
        for (int i = 0; i < 8; i++) {
            ieee |= ((uint64_t)gp_addr->ieee.ieee_addr[i]) << (i * 8);
        }
        return ieee;
    }
}
...
static esp_err_t gp_process_frame(const zb_gp_device_t *device, const zb_gp_frame_t *frame)
{
    ...
    /* Build JSON state and merge into device registry */
    char json[512];
    int json_len = gp_build_state_json(device, frame, json, sizeof(json));
    if (json_len < 0) {
        ESP_LOGW(TAG, "Failed to build GP state JSON");
        return ESP_FAIL;
    }

    uint64_t ieee = gp_get_ieee_addr(&device->gp_addr);
    cJSON *parsed = cJSON_Parse(json);
    if (parsed != NULL) {
        device_registry_merge_state(ieee, parsed);
        cJSON_Delete(parsed);
    }
    ...
    evt_device_state_t evt = {
        .ieee_addr = ieee,
        .short_addr = 0xFFFF,  /* GP devices don't have network short address */
        .endpoint = (device->gp_addr.app_id == ZB_GP_APP_ID_IEEE) ?
                    device->gp_addr.ieee.endpoint : ZB_GP_ENDPOINT,
        .cluster_id = ZB_GP_CLUSTER_ID,
        .attr_id = 0,  /* GP uses commands, not attributes */
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Failed to publish GP state event: %s", esp_err_to_name(ret));
    } else {
        ESP_LOGD(TAG, "Published GP state for %s: %s", device->friendly_name, json);
    }
                                      ...
                                      const zb_gp_frame_t *frame)
{
    ...
    /* Build state JSON and merge into device registry */
    char json[512];
    int json_len = gp_build_state_json(device, frame, json, sizeof(json));
    if (json_len < 0) {
        ESP_LOGW(TAG, "Failed to build GP state JSON");
