...
esp_err_t zb_backup_init(void)
{
    ...
    /* Mount LittleFS for backup storage */
    esp_err_t ret = mount_littlefs();
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "LittleFS mount failed: %s (backups will use NVS fallback)",
                 esp_err_to_name(ret));
...
esp_err_t zb_backup_save(const zb_backup_t *backup, char *filename, size_t filename_len)
{
    ...
    ESP_LOGI(TAG, "Saving backup to LittleFS...");
...
esp_err_t zb_backup_restore_from_file(const char *filename, zb_backup_restore_result_t *result)
{
    ...
    zb_backup_t *backup = (zb_backup_t *)mem_alloc(sizeof(zb_backup_t), MEM_CAP_PSRAM);
    if (backup == NULL) {
                                      ...
                                      const char *transaction_id)
{
    ...
    /* Publish via Event Bus for decoupled architecture */
    evt_backup_response_t evt = {
        .operation = BACKUP_OP_SAVE,
        .success = (status == ZB_BACKUP_STATUS_SUCCESS),
        .filename = NULL,
        .error_msg = (status != ZB_BACKUP_STATUS_SUCCESS) ? zb_backup_status_str(status) : NULL,
        .file_size = 0,
        .json_payload = json_str,
    };
    event_publish(EVT_BACKUP_RESPONSE, &evt, sizeof(evt));
    free(json_str);
                                              ...
                                              const char *transaction_id)
{
    ...
    /* Publish via Event Bus for decoupled architecture */
    evt_backup_response_t evt = {
        .operation = BACKUP_OP_RESTORE,
        .success = result->success,
        .filename = NULL,
        .error_msg = !result->success ? zb_backup_status_str(result->status) : NULL,
        .file_size = 0,
        .json_payload = json_str,
    };
    event_publish(EVT_BACKUP_RESPONSE, &evt, sizeof(evt));
...
static esp_err_t mount_littlefs(void)
{
    ESP_LOGI(TAG, "Mounting LittleFS for backup storage...");
...
static esp_err_t collect_devices(zb_backup_device_t *devices, uint16_t *count)
{
    ...
    *count = 0;
    ...
    for (size_t i = 0; i < dev_count && *count < ZB_BACKUP_MAX_DEVICES; i++) {
        ...
        /* Copy IEEE address from device_t id */
        bkp_dev->ieee_addr = dev->id;
        bkp_dev->short_addr = dev->proto.zigbee.short_addr;
        bkp_dev->endpoint = dev->proto.zigbee.endpoint;
        ...
        bkp_dev->device_type = 0;  /* device_t uses capabilities instead of device_type */
        bkp_dev->cluster_count = dev->proto.zigbee.cluster_count;

        /* Copy clusters */
        size_t cluster_copy = (dev->proto.zigbee.cluster_count < ZB_BACKUP_MAX_CLUSTERS) ?
                              dev->proto.zigbee.cluster_count : ZB_BACKUP_MAX_CLUSTERS;
        memcpy(bkp_dev->clusters, dev->proto.zigbee.clusters, cluster_copy * sizeof(uint16_t));
                                  ...
                                  zb_backup_restore_result_t *result)
{
    ...
    for (uint16_t i = 0; i < count; i++) {
        const zb_backup_device_t *backup_dev = &devices[i];

        /* Add device to NG registry using ieee_addr (already uint64_t) */
        device_t *ng_dev = device_registry_add(backup_dev->ieee_addr, DEV_PROTOCOL_ZIGBEE);
        if (ng_dev == NULL) {
            /* Check if device already exists */
            ng_dev = device_registry_get(backup_dev->ieee_addr);
            if (ng_dev == NULL) {
                ESP_LOGW(TAG, "Failed to restore device 0x%04X", backup_dev->short_addr);
                continue;
            }
        }

        /* Set device properties directly on NG device */
        ng_dev->proto.zigbee.short_addr = backup_dev->short_addr;
        ng_dev->proto.zigbee.endpoint = backup_dev->endpoint;
        ng_dev->proto.zigbee.device_type = (device_zigbee_type_t)backup_dev->device_type;

        /* Copy friendly name, manufacturer, model */
        if (backup_dev->friendly_name[0] != '\0') {
            strncpy(ng_dev->friendly_name, backup_dev->friendly_name, sizeof(ng_dev->friendly_name) - 1);
            ng_dev->friendly_name[sizeof(ng_dev->friendly_name) - 1] = '\0';
        }
        if (backup_dev->manufacturer[0] != '\0') {
            strncpy(ng_dev->manufacturer, backup_dev->manufacturer, sizeof(ng_dev->manufacturer) - 1);
            ng_dev->manufacturer[sizeof(ng_dev->manufacturer) - 1] = '\0';
        }
        if (backup_dev->model[0] != '\0') {
            strncpy(ng_dev->model, backup_dev->model, sizeof(ng_dev->model) - 1);
            ng_dev->model[sizeof(ng_dev->model) - 1] = '\0';
        }

        /* Add clusters to NG registry */
        for (uint16_t c = 0; c < backup_dev->cluster_count && c < ZB_BACKUP_MAX_CLUSTERS; c++) {
            device_zigbee_add_cluster(ng_dev, backup_dev->clusters[c]);
        }

        result->restored_devices++;
        ESP_LOGI(TAG, "Restored device 0x%04X (%s)", backup_dev->short_addr,
                 backup_dev->friendly_name[0] ? backup_dev->friendly_name : "unnamed");
    }
