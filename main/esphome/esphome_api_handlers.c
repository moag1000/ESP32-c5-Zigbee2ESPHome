...
static esp_err_t handle_execute_service(esphome_client_t *client, const esphome_message_t *msg)
{
     ...
     *
     * An argument holding its type's default value arrives as an empty
     * submessage, so `permit_join(0)` and `remove_device("")` carry no type
     * information at all. Taking it from the service definition also means a
     * client that sends the wrong field cannot steer a callback into reading
     * the wrong union member. */
    const esphome_service_t *svc = esphome_service_get_by_key(key);
    if (svc != NULL) {
        for (size_t i = 0; i < arg_count && i < svc->arg_count; i++) {
            if (!arg_present[i]) {
                args[i] = (esphome_service_arg_value_t){0};
            }
            args[i].type = svc->args[i].type;
        }
    }

    ESP_LOGI(TAG, "Execute service: key=%lu, arg_count=%zu",
             (unsigned long)key, arg_count);

    /* Publish command event for tracking */
    char json_buf[48];
    snprintf(json_buf, sizeof(json_buf), "{\"service_key\":%lu,\"arg_count\":%zu}",
             (unsigned long)key, arg_count);
