...
/** @brief WiFi connection wait timeout per attempt.
 *  10s is enough for DHCP — if it fails, the retry succeeds immediately.
 *  Previous 45s wasted ~35s on every boot when first DHCP attempt stalled. */
#define GW_DEFAULT_WIFI_TIMEOUT_MS          10000
...
#define GW_TASK_STACK_MQTT                  8192

/** @brief OTA task stack size (bytes) */
#define GW_TASK_STACK_OTA                   8192

/** @brief MQTT outbox memory limit (bytes)
 * After reconnect, burst of availability + bridge info + device list +
 * bridge discovery (~25+ messages) can exceed 16KB before PUBACKs drain. */
#define GW_MQTT_OUTBOX_LIMIT                32768
