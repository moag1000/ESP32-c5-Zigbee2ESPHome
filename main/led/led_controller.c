...
esp_err_t led_controller_init(void)
{
    ...
    /* LED Strip API configuration
     * v2.5.x uses led_pixel_format, v3.0+ uses color_component_format
     * Check for LED_STRIP_COLOR_COMPONENT_FMT_RGB macro to detect v3.0+ API */
#if defined(LED_STRIP_COLOR_COMPONENT_FMT_RGB)
    /* led_strip v3.0+ API */
...
void led_blink_notify(led_notify_t notify)
{
    switch (notify) {
        case LED_NOTIFY_DEVICE_NEW:
            ...
            ESP_LOGI(TAG, "Notification: New device (1x red)");
            break;
